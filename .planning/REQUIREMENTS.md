...
- [ ] **ENC-02**: When a passphrase is entered, the exported file is AES-256-GCM encrypted with PBKDF2 key derivation and a random salt
- [ ] **ENC-03**: The encrypted file uses a self-describing envelope `{ encrypted, version, salt, iv, data }` so import can auto-detect it
